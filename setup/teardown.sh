...
if [ ! -d $OLD_DOT_FILES ]; then
    ...
    for f in .bashrc .bash_profile .inputrc .vimrc .vrapperrc .editrc .bash_prompt .bash_history .gitconfig .gitignore_global .zshrc .tmux.conf .shellrc .shell_colors .shell_functions .shell_aliases .shell_path .nvimrc
    do
        if [ -h ~/$f ] || [ -e ~/$f ] ; then
            rm ~/$f
        fi
